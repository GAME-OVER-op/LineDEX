...
final class PhoneControlPanelLauncher {
    ...
    static void open(final Activity source) {
        ...
        if (crossDisplay && ShellAccess.isReady()) {
            final String command = createLaunchCommand(
                    ...
                    LineDexControlActivity.class.getName());
            EXECUTOR.execute(() -> {
                try {
                    final String output =
    ...
    private static void openWithAndroidApi(final Activity source) {
        ...
        final Intent intent = new Intent(source, LineDexControlActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_SINGLE_TOP);
